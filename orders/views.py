...
class CartView(APIView):
    ...
    def get(self, request):
        ...
        total_amount = qs.aggregate(
            total=Sum(F('qty') * F('price_at_add'))
        )['total'] or 0
...
class AddToCartView(APIView):
    ...
    def post(self, request):
        ...
        if request.user.is_authenticated:
            ...
            sk = request.session.session_key  # уже гарантирован в get_cart_qs
            item, created = CartItem.objects.get_or_create(
...
class BasketView(APIView):
    ...
    def get(self, request):
        ...
        data = {'items': qs, 'total_qty': total_qty, 'total_amount': total_amount}
        return Response(CartSerializer(data).data)
